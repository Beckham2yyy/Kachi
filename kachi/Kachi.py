...
import time
import traceback
from aiohttp import web  # added only for web service
...
async def main():
    ...
    await send_telegram("Sniping 🍁")  # first message
...
# =========================
# WEB SERVICE HACK FOR RENDER
# =========================

async def handle(request):
    return web.Response(text="Kachi is running")

app = web.Application()
app.router.add_get("/", handle)

# ensure Telegram sends reliably on startup
async def startup_message():
    await asyncio.sleep(1)  # tiny delay for event loop readiness
    await send_telegram("Sniping 🍁")

loop = asyncio.get_event_loop()
loop.create_task(startup_message())  # first message guaranteed
